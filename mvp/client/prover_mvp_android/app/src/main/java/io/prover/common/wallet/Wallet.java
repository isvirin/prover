package io.prover.common.wallet;
...
import static io.prover.common.wallet.SecureRandomUtils.secureRandom;
...
public class Wallet {
    ...
    private static WalletFile createWalletFile(
        ...
        WalletFile.ScryptKdfParams kdfParams = new WalletFile.ScryptKdfParams(DKLEN, n, p, R, salt);
        WalletFile.CipherParams cipherParams = new WalletFile.CipherParams(iv);

        WalletFile.Crypto crypto = new WalletFile.Crypto(CIPHER, cipherText, cipherParams, SCRYPT, kdfParams, mac);

        return new WalletFile(crypto, UUID.randomUUID().toString(), CURRENT_VERSION);
    }
    ...
    public static ECKeyPair decrypt(String password, WalletFile walletFile)
        ...
        WalletFile.Crypto crypto = walletFile.crypto;

        byte[] mac = Numeric.hexStringToByteArray(crypto.mac);
        byte[] iv = Numeric.hexStringToByteArray(crypto.cipherparams.iv);
        byte[] cipherText = Numeric.hexStringToByteArray(crypto.ciphertext);

        byte[] derivedKey;

        WalletFile.KdfParams kdfParams = crypto.kdfparams;
        if (kdfParams instanceof WalletFile.ScryptKdfParams) {
            WalletFile.ScryptKdfParams scryptKdfParams =
                    (WalletFile.ScryptKdfParams) crypto.kdfparams;
            int dklen = scryptKdfParams.dklen;
            int n = scryptKdfParams.n;
            int p = scryptKdfParams.p;
            int r = scryptKdfParams.r;
            byte[] salt = Numeric.hexStringToByteArray(scryptKdfParams.salt);
            derivedKey = generateDerivedScryptKey(
                    password.getBytes(Charset.forName("UTF-8")), salt, n, r, p, dklen);
        } else if (kdfParams instanceof WalletFile.Aes128CtrKdfParams) {
            WalletFile.Aes128CtrKdfParams aes128CtrKdfParams =
                    (WalletFile.Aes128CtrKdfParams) crypto.kdfparams;
            int c = aes128CtrKdfParams.c;
            String prf = aes128CtrKdfParams.prf;
            byte[] salt = Numeric.hexStringToByteArray(aes128CtrKdfParams.salt);
