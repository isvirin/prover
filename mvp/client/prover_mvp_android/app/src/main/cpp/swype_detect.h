...
public:
    ...
    void
    processFrame(cv::Mat frame, int &state, int &index, int &x, int &y);   // a frame processing
    void processFrame(const unsigned char *frame_in, int width, int height, int &state, int &index,
                      int &x, int &y);
